...
document.addEventListener("DOMContentLoaded", () => {
  // 상세 모달 로드
  const postTab = document.querySelector("#tab-posts");
  postTab.querySelectorAll(".post-card").forEach((card) => {
  ...
  // 숫자 포맷팅
  postTab.querySelectorAll(".count").forEach((el) => {
    const n = el.getAttribute("data-count");
    el.textContent = formatCount(n);
  });

  // 좋아요 토글: 서버 응답으로 로그인 판별
  postTab.querySelectorAll(".btn-like").forEach((btn) => {
  ...
  postTab.querySelectorAll(".btn-bookmark").forEach((btn) => {
  ...
  // 댓글 버튼 = 카드 클릭
  postTab.querySelectorAll(".btn-comment").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();
      e.preventDefault();
...
document.addEventListener("DOMContentLoaded", () => {
  // 상세 모달 로드
  const bookmarkTab = document.querySelector("#tab-bookmarks");
  bookmarkTab.querySelectorAll(".post-card").forEach((card) => {
    card.addEventListener("click", async (e) => {
      // 액션 버튼 클릭은 무시
      if (e.target.closest(".btn-action")) return;
  ...
  // 숫자 포맷팅
  bookmarkTab.querySelectorAll(".count").forEach((el) => {
    const n = el.getAttribute("data-count");
    el.textContent = formatCount(n);
  });

  // 좋아요 토글: 서버 응답으로 로그인 판별
  bookmarkTab.querySelectorAll(".btn-like").forEach((btn) => {
    btn.addEventListener("click", function (e) {
      e.stopPropagation();
      e.preventDefault();
  ...
  // 북마크 토글: 서버 응답으로 로그인 판별
  bookmarkTab.querySelectorAll(".btn-bookmark").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();
      e.preventDefault();
  ...
  // 댓글 버튼 = 카드 클릭
  bookmarkTab.querySelectorAll(".btn-comment").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();
      e.preventDefault();
      btn.closest(".post-card")?.click();
