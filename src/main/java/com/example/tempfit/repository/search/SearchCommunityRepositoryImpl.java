    ...
    public Page<Object[]> list(String type, String keyword, List<String> styleNames, Pageable pageable) {
        ...
        JPQLQuery<Tuple> query = from(community)
            ...
            .select(
                ...
                style.street,
                style.formal,
                style.outdoor            // 변경: style.sports → style.outdoor
            )
            .distinct();
        ...
        if (styleNames != null && !styleNames.isEmpty()) {
            ...
            for (String s : styleNames) {
                switch (s.toUpperCase()) {
                    ...
                    case "OUTDOOR": sb.or(style.outdoor.isTrue()); break;  // 변경
                }
            }
            builder.and(sb);
        }
        ...
        List<Object[]> results = tuples.stream()
            .map(t -> new Object[]{
                ...
                t.get(style.street),
                t.get(style.formal),
                t.get(style.outdoor)      // 변경: t.get(style.sports) → t.get(style.outdoor)
            })
